...
class OrderController extends Controller
{
    ...
    {
        if ($order->status->order_status == "Selesai") {
            $message = "The order has already succeded by admin!";
            myFlasherBuilder(message: $message, failed: true);
            return redirect("/order/order_data");
        }
        if ($order->status->order_status !== "Disetujui") {
    ...
    public function uploadProof(Request $request, Order $order)
    {
        ...
        if ($request->file("image_upload_proof")) {
            ...
            $imagePath = $request->file('image_upload_proof')->store('public/proof');
    ...
    public function editOrderPost(Request $request, Order $order)
    {
        ...
        if ($request->file("image_proof_edit")) {
            if ($order->transaction_doc != env("IMAGE_PROOF")) {
                Storage::delete('public/' . $order->transaction_doc);
            }
            $imagePath = $request->file('image_proof_edit')->store('public/proof');
            $validatedData['transaction_doc'] = str_replace('public/', '', $imagePath);
        }
